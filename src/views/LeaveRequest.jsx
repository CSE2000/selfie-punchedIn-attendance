...
const LeaveRequestPage = () => {
  ...
  const getLeaveRequests = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.get("http://192.168.1.26:8000/userleave", {
        headers: {
      ...
      // Make sure to handle different possible response structures
      const leaveData =
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://192.168.1.26:8000/userleave",
        payload,
        {
          headers: {
...
//       console.log("Fetched Leave Requests:", response.data);
//       setLeaveRequests(response.data.leaveData || []);
//     } catch (err) {
//       console.error("GET Error:", err);
//       setError(err.response?.data?.message || "Failed to load leave requests");
//     }
...
//       setEndDate(null);
//       setReason("");
//       await getLeaveRequests(); // Refresh data
//     } catch (err) {
//       console.error("POST Error:", err);
//       setError(err.response?.data?.message || "Failed to submit leave request");
//     }
...
//           ?.filter(
//             (leave) =>
//               (tab === "Approved" && leave.status === "approved") ||
//               (tab === "Rejected" && leave.status === "rejected") ||
...
//                     </div>
//                     <div className="text-gray-500 text-xs mt-1">
//                       Reason: <span className="italic">{leave.reason}</span>
//                     </div>
//                   </div>
//                   <div className="text-right">
//                     <div className="text-xs text-gray-400">Total Leave</div>
...
//         <button
//           onClick={handleSubmit}
//           className="w-full mt-4 bg-[#7A5AF8] text-white py-2 rounded shadow-md hover:bg-purple-700"
//         >
//           Submit
//         </button>
//       </div>
...
//   return (
//     <div className="max-w-md mx-auto bg-white rounded-lg shadow-md text-sm">
//       <div className="flex justify-between items-center gap-4">
//         <div className="w-1/2">
//           <label className="text-gray-600 text-xs">Select Date</label>
//           <div className="mt-1 border border-gray-300 rounded px-2 py-1 text-gray-700">
...
//                 </span>
//                 <span>
//                   {tab} at {format(new Date(), "dd MMM yyyy")}
//                 </span>
//               </div>
//               <div className="flex items-center gap-2">
//                 <img src={Hr} alt="User" className="w-6 h-6 rounded-full" />
//                 <span className="text-gray-500 text-xs">By Pradip</span>
//               </div>
//             </div>
//           </div>
//         )}
//         <button className="w-full mt-4 bg-[#7A5AF8] text-white py-2 rounded shadow-md hover:bg-purple-700">
//           Submit
//         </button>
//       </div>
//     </div>
