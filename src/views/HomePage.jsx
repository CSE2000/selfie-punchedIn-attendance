  ...
  useEffect(() => {
    const fetchAttendance = async () => {
      try {
        ...
        // Transform the API response using backend data directly
        const transformedData = apiResponse.punchData.map((entry) => {
          ...
          // Use backend status directly and map to display format
          ...
          // Check if user is currently punched in (has punchIn but no punchOut for today)
          ...
          if (isToday && !hasPunchOut) {
            ...
            // For currently punched in users, show entry timing status
          ...
          return {
            ...
            entryTime: entry.entryTime, // Keep original entry time info
          };
        });
        ...
        if (selectedStatus !== "All") {
          filteredData = transformedData.filter((entry) => {
            if (selectedStatus === "Late") {
              return entry.status === "Late" || entry.status === "On Time";
            }
        ...
        // Calculate summary from API response or from current data
        if (apiResponse.summary) {
          setAttendanceSummary({
            present: apiResponse.summary.present || 0,
            leave: (apiResponse.summary.halfday || 0) + (apiResponse.summary.late || 0),
            absent: apiResponse.summary.absent || 0,
          });
        } else {
          // Calculate from transformed data
          const summary = {
            present: filteredData.filter((d) => 
              d.status === "Present" || d.status === "On Time"
            ).length,
            leave: filteredData.filter((d) => 
              d.status === "Halfday" || d.status === "Late"
            ).length,
...
//   // Determine if current punch-in time is late (after 9:30 AM)
//   const isCurrentPunchLate = (punchTime) => {
//     const punch = new Date(punchTime);
//     const cutoffTime = new Date(punch);
//     cutoffTime.setHours(9, 30, 0, 0); // 9:30 AM
//     return punch > cutoffTime;
//   };
...
//     // Determine if punch-in is late
//     const isLate = isCurrentPunchLate(now);

//     const attendanceEntry = {
//       date: todayStr,
//       status: "Present", // Always Present when punched in (final status determined after punch out)
//       punchIn:
//         punchInTimeStr.split(":")[0] + ":" + punchInTimeStr.split(":")[1],
//       punchOut: "Out Pending",
//       entryTime: isLate ? "late" : "ontime",
//       isToday: true, // Flag to identify today's live entry
//     };
...
//         `${todayAttendance.date}T${todayAttendance.punchIn}:00`
//       );
//       const punchOutDate = new Date(
//         `${todayAttendance.date}T${punchOutTime}:00`
//       );

//       // Calculate total hours worked
//       const hoursWorked = (punchOutDate - punchInDate) / (1000 * 60 * 60);

//       // Determine final status based on hours worked and entry time
...
//         finalStatus = todayAttendance.entryTime === "late" ? "Late" : "Present";
//       } else {
//         finalStatus = "Halfday";
//       }
...
//         punchOut: punchOutTime.split(":")[0] + ":" + punchOutTime.split(":")[1],
//         status: finalStatus,
//         isToday: false, // No longer active session
//       };
...
//           // Use backend status directly - backend determines final status
//           let displayStatus = "Present";
//           if (entry.status === "absent") {
//             displayStatus = "Absent";
//           } else if (entry.status === "present") {
...
//           setAttendanceSummary({
//             present:
//               (apiResponse.summary.present || 0) +
//               (apiResponse.summary.ontime || 0),
//             leave:
//               (apiResponse.summary.halfday || 0) +
//               (apiResponse.summary.late || 0),
//             absent: apiResponse.summary.absent || 0,
...
//                     todayAttendance.entryTime === "ontime"
...
//               {/* Right side: Status and timing info */}
//               <div className="flex flex-col items-end text-right space-y-1">
//                 <div className="flex items-center space-x-2">
//                   <span
//                     className={`font-medium ${getStatusColor(entry.status)}`}
//                   >
//                     {entry.status}
//                   </span>
//                   {/* Show entry time indicator ONLY for today's active session */}
//                   {entry.isToday && isPunchedIn && entry.entryTime && (
//                     <span
//                       className={`text-xs px-2 py-1 rounded-full ${
//                         entry.entryTime === "ontime"
//                           ? "bg-green-100 text-green-700"
//                           : "bg-orange-100 text-orange-700"
//                       }`}
//                     >
//                       {entry.entryTime === "ontime" ? "On Time" : "Late"}
//                     </span>
//                   )}
//                 </div>
//                 <span className="text-xs text-gray-600">
