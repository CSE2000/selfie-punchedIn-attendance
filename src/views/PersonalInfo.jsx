...
const PersonalInfoForm = () => {
  ...
  const BASE_API_ENDPOINT = "https://attendancebackends.onrender.com/user/me";
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (userAuthData?.image) {
          setImagePreview(userAuthData.image);
        }
  ...
  const sections = {
    ...
    employment: {
      fields: [{ key: "image", label: "Upload your photo", type: "file" }],
    },
  };
  ...
  const handleInputChange = (key, value) => {
    ...
    // Handle image preview for display only
    if (key === "image" && value) {
      const file = value;
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Prepare updated data without file upload
      const updatedData = {
        ...userData,
        userId: userId,
      };

      // Remove any file objects from data
      delete updatedData.photo;
      if (selectedImageFile) {
        // Don't include the file object in the request
        ...
        <button
          onClick={() => {
            if (currentTabIndex === 0) {
              ...
              setSection(tabs[currentTabIndex - 1].id);
            }
          }}
